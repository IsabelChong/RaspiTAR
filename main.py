...
def print_lcd(tempp):
    ...
    if len(Name) > 16:
        cutName = Name[0:16]
        mylcd.lcd_display_string(cutName, 1,0)
    else:
        mylcd.lcd_display_string(Name, 1,0)
        
    mylcd.lcd_display_string("ID:", 2,0)
    mylcd.lcd_display_string("T:", 2,9)
    mylcd.lcd_display_string(str(chr(223)), 2,15)
    mylcd.lcd_display_string(StudID, 2,3)
    mylcd.lcd_display_string(str(tempp), 2,11)
################################################### DataReader (QThread)
...
class MainWindow(QMainWindow):
    def __init__(self, parent = None, *args, **kwargs):
        ...
        self.idLabel = QLabel("Student ID (Last 4 Digits):")
        self.idLabel.setFont(QFont('Arial', 16))
        self.idInputLabel = QLabel("Waiting for Input...")
        self.idInputLabel.setFont(QFont('Arial', 16))
        ...
        self.gBox.addWidget(self.idLabel, 3,0)
        self.gBox.addWidget(self.idInputLabel, 3,1)
        self.gBox.addWidget(self.classLabel, 4,0)
        self.gBox.addWidget(self.timeLabel, 4,1)
        self.gBox.addWidget(self.dateLabel, 5,0, 1,0)
        self.gBox.addWidget(self.sendAttendanceButton, 6,0)
        self.gBox.addWidget(self.quitButton, 6,1)
    ...
    def execute_this_fn(self, current_frame): 
        ...
        try:
            ...
            cv2.imwrite('/home/pi/Desktop/RaspiTAR_App/LocalFile/Image.jpg', image)
            ...
            splitvar = iniName.split("-", 1)[0]
            StudID = iniName.split("-", 1)[1]
            Name = splitvar.replace("_", " ")
            return current_frame
                
        except:
            print("No match/No internet connection")
    ...
    def print_output(self, s):
        ...
        try:
            ...
            if Name is not preName:
                self.nameInputLabel.setText(Name)
                self.idInputLabel.setText(StudID)
                
                snapimage = self.convert_cv_qt(s)
                self.snapLabel.setPixmap(snapimage)
                self.tempInputLabel.setText(str(temp)+ "°")
                x = threading.Thread(target=print_lcd, args=(temp,)) #thread not working
                x.start()
                preName = Name
            else:
                pass
            ...
            try:
                AddItems(current_class, StudID, Name, temp).start()
            except:
                pass
            
            if temp > 37.5:
                SESEmail(ts.strftime("%Y-%m-%d %H:%M"), Name, StudID, temp, current_class).start()
